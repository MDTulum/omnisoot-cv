...
def simulate_flowreactor(
    ...
    flow_columns += ["t[s]", "z[m]", "T[K]", "density[kg/m3]", "mean_MW[kg/mol]", "velocity[m/s]",
                    "k_gas[W/m-K]", "cp_gas[J/kg-K]", "mu_gas[Pa-s]"]
    flow_data += [states.restime, states.z, states.T, states.density,
                states.mean_molecular_weight/1000, states.velocity,
                states.k_gas, states.cp_gas, states.mu_gas]
    ...
    output_dir = f"results/Adiabatic";
    if not os.path.exists(output_dir):
        os.makedirs(output_dir);

    temp0_c = temperature_profile.T[0] - 273.15;
